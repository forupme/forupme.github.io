...
為何每個團隊只需要三個 IPv4 addresses 用來配置伺服器. 

一個 IPv4 address 用於 DNS 伺服器，一個 IPv4 address 用於 Proxy 伺服器，
假如還不夠，那就再一個 IPv4 address 用於第二台 Proxy 伺服器。
而院或系則選一台伺服器，安裝 DHCP6，專門在共用網段中分配 IPv6 網址，
另外則要求所有軟體的認證伺服器全部支援 IPv6 協定。

如此，所有客戶端的 IPv4 網路協定全部都可以關閉。
